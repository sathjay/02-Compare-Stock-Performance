...
start_date=dt(2018, 1, 1).date()  # Default start date to get the price quotes
...
def get_Data(ticker_list, start, end):
    ...
    df_close = df['Adj Close'] # The above df_adj_close more columns are added to calculate cummilative return.
    ...
    stock_cummilative_return = df_adj_close[df_adj_close.columns[pd.Series(df_adj_close.columns).str.startswith('CR_')]]
    stock_cummilative_return.rename(columns = lambda x: x.strip('CR_'),inplace = True)
...
app = dash.Dash(__name__, external_stylesheets=external_stylesheets,title = "Stock Comparision")
server = app.server
...
app.layout = html.Div([
                        ...
                        html.H4('Compare 4 stock return perfomance:'),
                        
                        html.P('For the four stocks selected from the below drop down, users can compare their \
                               daily return distribution, 60 days rolling volatilty, 60 days rolling Sharpe ratio, \
                               60 days Sortino volatitly, and 60 days Rolling Sortino ratio.'),
...
def process_selected(ticker_list):
    ...
    if len(ticker_list) != 4:
        message = html.P('Please select 4 stocks from the dropdown list above', style = {'color':'red','font-size':'15px'})
        
        return message,fig,stock_cum_ret_fig,rolling_vol_fig,rolling_SR_fig,rolling_sortino_vol_fig,rolling_sortino_ratio_fig
    
    else:
        message = ['You selected ' + ', '.join(str(stock) for stock in ticker_list)]
        return_summary, df_close,stock_daily_return,stock_cummilative_return,annualized_std,log_stock_daily_returns= get_Data(ticker_list, start_date,end_date )
        ...
        for stock in stock_list:
            trace = go.Histogram(x=log_stock_daily_returns[stock]*100, name = stock)
            traces.append(trace)
        ...
        fig.update_layout(autosize = False,  
                          title = dict(text = 'Frequency of Daily log returns in %'),
                  ...
                  yaxis=dict(showline = True,linecolor='black',showgrid = True),
                  yaxis2=dict(showline = True,linecolor='black',showgrid = True),
                  yaxis3=dict(showline = True,linecolor='black',showgrid = True),
                  yaxis4=dict(showline = True,linecolor='black',showgrid = True)
        ...
        TRADING_DAYS = 60
        volatility = log_stock_daily_returns.rolling(window=TRADING_DAYS).std()*np.sqrt(TRADING_DAYS)*100
        volatility.tail()
        rolling_vol_data = []
        ...
        sharpe_ratio = (log_stock_daily_returns.rolling(window=TRADING_DAYS).mean() - Rf)*TRADING_DAYS / volatility*100
        ...
        rolling_SR_fig = go.Figure(data=rolling_SR_data, layout=rolling_SR_layout)
        
        
        #60 day rolling Sortino volatility
        
        sortino_vol = log_stock_daily_returns[log_stock_daily_returns<0].rolling(window=TRADING_DAYS, center=True, min_periods=10).std()*np.sqrt(TRADING_DAYS)*100
        sortino_ratio = (log_stock_daily_returns.rolling(window=TRADING_DAYS).mean() - Rf)*TRADING_DAYS / sortino_vol*100
        ...
        rolling_sortino_vol_fig = go.Figure(data=rolling_sortino_vol_data, layout=rolling_sortino_vol_layout)
        ...
        rolling_sortino_ratio_fig = go.Figure(data=rolling_sortino_ratio_data, layout=rolling_sortino_ratio_layout)
